...
`[인사]`, `[부고]`, `[동정]`, `[오늘의 주요일정]` 등으로 시작하는 제목은 여러 매체가 그대로 받아쓰는 정형 공지성 콘텐츠라 보도 매체 수는 많아도 뉴스 가치가 낮습니다. `scripts/common.py`의 `LOW_VALUE_TITLE_PATTERNS`에서 관리하며, 수집 단계(`collect_hourly.py`)와 요약 단계(`summarize_daily.py`) 양쪽에서 걸러냅니다. 새로운 패턴이 보이면 이 목록에 정규식을 추가하면 됩니다.
